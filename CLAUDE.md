...
- **API routes**: `api/auth/[...all]` (better-auth handler), `api/export` (CSV export of the current user's rows), and `api/whatsapp` (bot webhook).
- **WhatsApp bot** (`lib/whatsapp/`): chat interface to the same service layer, through a self-hosted [go-whatsapp-web-multidevice](https://github.com/aldinokemal/go-whatsapp-web-multidevice) (GOWA) instance. `client.ts` sends via GOWA's REST API, `webhook.ts` verifies the `X-Hub-Signature-256` HMAC over the **raw** body (never `request.json()`) and filters out non-`message` events, `is_from_me`, and group chats. `bot.ts` routes a message; `intent.ts` turns free text into a zod-validated intent via `lib/llm.ts`. The LLM never touches the DB — every action goes through `lib/finances.ts` filtered by `userId`. WhatsApp has no buttons, so undo and edit/delete confirmation are text replies backed by short-lived Redis state in `state.ts`. Access is gated in a fixed order (signature, event filters, dedupe, `HUBUNGKAN`, `chat_link` lookup, per-JID rate limit, non-LLM commands, daily LLM quota); see `plans/plan_whatsapp.md`.
- **Account linking**: `chat_link` maps one account to one WhatsApp identity. WhatsApp exposes both a phone JID (`628...@s.whatsapp.net`) and a LID (`...@lid`), so both columns are matched against both values on lookup. One-time codes live in Redis, and the dashboard hands out a `wa.me/<nomor>?text=HUBUNGKAN <kode>` link.
